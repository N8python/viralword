function format(number: number, decPlaces = 2): string {
    let ret: string = "";
    // 2 decimal places => 100, 3 => 1000, etc
    decPlaces = Math.pow(10, decPlaces);
  
    // Enumerate number abbreviations
    var abbrev: string[] = [" thousand", " million", "B", "T", "q", "Q", "s", "S", "O", "N", "D"];
...
function fame(percent: number): string {
    if (percent < 0.05) {
...
const celeb: HTMLSelectElement | null = document.querySelector("[name=\"celeb\"]");
const word: HTMLInputElement | null = document.querySelector("input[type=\"text\"]");
const submit: HTMLButtonElement | null = document.querySelector("button[type=\"submit\"]");
const viewArea: HTMLDivElement | null = document.querySelector("#views");
const canvas: HTMLCanvasElement | null = document.querySelector("#america");
const ctx: CanvasRenderingContext2D | null | undefined = canvas ? canvas.getContext("2d") : void 0;
const america: HTMLImageElement | null = document.querySelector("[src=\"america.jpeg\"");
const inputForm: HTMLElement | null = document.querySelector("#inputForm");
const viewDisplay: HTMLParagraphElement | null = document.querySelector("#totalViews");
const result: HTMLParagraphElement | null = document.querySelector("#result");
const usPopulation: number = 3.272e8;
let views: number = 0;
let finalViews: number = 0;
let followers: number = 0;
let viewMomentum: number = 1;
let viewId: number;
let percent: number;
function viewAmount(followers: number): number {
    let viewsCount: number = 0;
    viewsCount+=followers/20;
    let retweets: number = followers*0.00028;
    let retweet: boolean = Math.random() < retweets;
...
const celebsToFollowers: Dictionary<number> = {
    "Katy Perry": 1.07e8,
    "Barack Obama": 1.05e8,
                ...
                if(ctx){
                    ...
                    viewId = setInterval(() => {
                        ...
                        if(randInt(1, views*(1000+viewMomentum/2)) < views*viewMomentum){
                            if(viewMomentum < 30) viewMomentum++;
